...
public class ValiDataGen
{
    public static final String in_prefix = "./datas/Semantic/";
    public static final String ou_prefix = "./datas/Semantic/ParaStudy/";
    ...
    {
        fileModel.CreateFolder(ou_prefix);
        try
        {
            FileWriter fw = new FileWriter(new File(ou_prefix + name + ".txt"));
    ...
    public static List<Integer> FindCenters_Exclude(
        ...
        {
            if(excl.contains(center)) continue;
            Map<Integer, Integer> count = new HashMap<>(); count.clear();
            Set<Integer> ansnodes = new HashSet<>(); ansnodes.clear();
            haswalked.clear();
            DFS(center, 0, paths.get(0), count);
        ...
        try
        {
            ...
            while(sc.hasNextLine())
            {
                ...
                else
                {
                    ...
                    Set<Integer> excl = new HashSet<>(); excl.clear();
                    for(Integer i = 0; i < cens.length; ++ i) excl.add(stoi(cens[i]));
                    centers = FindCenters_Exclude(paths, num_per_group, excl);
                }
