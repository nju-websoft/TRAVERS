...
public class Clicks {
    ...
    public static Pair<List<Integer>, List<Integer>> label(Pair<List<Integer>, Integer> ret, List<Integer> ans,
        ...
        for (Integer node : list)
            if (!A.contains(node) && !has_labeled.contains(node)) {
                neg.add(node);
                break;
            }
        ...
        if (!has_labeled.contains(ret.getValue())) {
            if (A.contains(ret.getValue()))
                pos.add(ret.getValue());
            else
                neg.add(ret.getValue());
        }
        if (pos.size() <= 0) {
            Collections.shuffle(ans);
            for (Integer node : ans)
                if (!has_labeled.contains(ans.get(node))) {
                    pos.add(node);
                    break;
                }
        }
        has_labeled.addAll(pos);
        has_labeled.addAll(neg);
    ...
    public static Pair<Set<Integer>, Set<Integer>> List2Set(Pair<List<Integer>, List<Integer>> ret) {
        Set<Integer> pos_c = new HashSet<>();
        pos_c.clear();
        Set<Integer> neg_c = new HashSet<>();
        neg_c.clear();
        pos_c.addAll(ret.getKey());
        neg_c.addAll(ret.getValue());
    ...
    public static Pair<Set<Integer>, Set<Integer>> ClickModel(Map<Integer, Integer> STD, List<Integer> list) {
        if (GlobalVariances.Click_xxplicit.equals("Explicit"))
            return List2Set(list_ClickModel_Explicit(STD, list));
        else
            return List2Set(list_ClickModel_Implicit(STD, list));
    }

    public static Pair<List<Integer>, List<Integer>> ClickModel_List(Map<Integer, Integer> STD, List<Integer> list) {
        if (GlobalVariances.Click_xxplicit.equals("Explicit"))
            return list_ClickModel_Explicit(STD, list);
        else
            return list_ClickModel_Implicit(STD, list);
    }
    ...
    public static Pair<List<Integer>, List<Integer>> list_ClickModel_Explicit(Map<Integer, Integer> STD,
            List<Integer> list) {
        LoadPara();
        List<Integer> pos_c = new ArrayList<>();
        pos_c.clear();
        List<Integer> neg_c = new ArrayList<>();
        neg_c.clear();
    ...
    public static Pair<List<Integer>, List<Integer>> list_ClickModel_Perfect(Map<Integer, Integer> STD,
            List<Integer> list) {
        LoadPara();
        List<Integer> pos_c = new ArrayList<>();
        pos_c.clear();
    ...
    public static Pair<List<Integer>, List<Integer>> list_ClickModel_Implicit(Map<Integer, Integer> STD,
            List<Integer> list) {
        LoadPara();
        List<Integer> pos_c = new ArrayList<>();
        pos_c.clear();
        ...
        for (Integer id = list.size(); id >= 1; --id) {
            ...
            if (click) {
                pos_c.add(list.get(id - 1));
                if (!flag && id < list.size())
                    neg_c.add(list.get(id));
                flag = true;
            } else if (flag)
                neg_c.add(list.get(id - 1));
        }
