...
class ClickModel(object):
  ...
  def generate_clicks(self, ranking, all_labels):
    ...
    labels = all_labels[ranking]
    coinflips = np.random.rand(*ranking.shape)
    observed_pro = np.random.rand(*ranking.shape)
    observed_threshold = np.array([(1.0 / (float(i + 1) ** self.PSTOP[0])) for i in range(ranking.shape[0])])
    obs = observed_pro < observed_threshold
    cli = coinflips < self.PCLICK[labels]
    return np.logical_and(obs, cli)
...
'''
# per = per, nav = random, inf = bin

bin_models = {}
bin_models['perfect'] = np.array([0.0, 1.0]), np.array([0.0, 0.0])
bin_models['informational'] = np.array([0.1, 1.0]), np.array([1.0, 1.0])
bin_models['navigational'] = np.array([0.4, 0.6]), np.array([1.0, 1.0])

all_models = {'binary': bin_models}
