...
class DataFold(object):
    ...
    def train_query_fea_mat(self, ranking_index):
      # print 'Train Matrix Shape : '
      # print self.train_feature_matrix.shape
      s_i = self.train_doclist_ranges[ranking_index]
      e_i = self.train_doclist_ranges[ranking_index+1]
      return self.train_feature_matrix[s_i:e_i, :]
    ...
    def read_data(self):
        ...
        self.coldstart = []
        for line in open(self.data_path + 'cold.txt', 'r'):
            self.coldstart.append(int(line))

        op = open(os.path.join(self.data_path, 'alpha.txt'), 'r')
        line = op.readline()
        line = line.replace('\r', '')
        line = line.replace('\n', '')
        self.alpha_result = [float(x) for x in line.split(' ')]
        op.close()
