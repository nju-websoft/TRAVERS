...
def evaluate_alpha(ranking, fea_mat, alpha_result, max_len):
    l = ranking.shape[0]
    features = fea_mat.shape[1]
    # print(fea_mat.shape)
    alpha_idcg = alpha_result[l - 1]
    rec = {}
    lis = []
    for id in range(l):
        label = ranking[id]
        ...
        for i in range(features):
            if int(fea_mat[label][i]) <= 0:
                continue
            if i not in rec.keys():
                rec[i] = 0.0
            s += (1.0 - alpha) ** rec[i]
            rec[i] += 1.0
        lis.append(s / math.log(id + 2))
    # print(np.sum(lis) / alpha_idcg)
    return np.sum(lis) / alpha_idcg
